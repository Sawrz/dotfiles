...
SERVICE_DIR=/home/jupyter/desk

# Setting this, so the repo does not need to be given on the command-line:
export BORG_REPO=/mnt/backups/desk

# See the section "Passphrase notes" for more info.
export BORG_PASSPHRASE=$(cat /home/sandro/.secrets/desk-backup.secret)
...
borg create                                                                                          \
    ...
    --exclude '/var/cache/*'                                                                         \
    --exclude '/var/tmp/*'                                                                           \
    --exclude $SERVICE_DIR'/.ipynb_checkpoints/*'						     \
                                                                                                     \
    ::'{hostname}-{now}'                                                                             \
    $SERVICE_DIR                                                                                     \
