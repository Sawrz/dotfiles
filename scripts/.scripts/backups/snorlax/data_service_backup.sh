...
SERVICE_DIR=/mnt/data/services

# Setting this, so the repo does not need to be given on the command-line:
export BORG_REPO=/mnt/vault/backups/pcs/snorlax/services/

# See the section "Passphrase notes" for more info.
export BORG_PASSPHRASE=$(cat /home/sandro/.secrets/data-services-backup.secret)
...
borg create                                                                                          \
    ...
    --exclude '/var/cache/*'                                                                         \
    --exclude '/var/tmp/*'                                                                           \
    --exclude $SERVICE_DIR'/plex/config/Library/Application Support/Plex Media Server/Cache/*'     \
    --exclude $SERVICE_DIR'/plex/config/Library/Application Support/Plex Media Server/Logs/*'      \
    --exclude $SERVICE_DIR'/openhab/data/logs'                                                     \
                                                                                                     \
    ::'{hostname}-{now}'                                                                             \
    $SERVICE_DIR                                                                                     \
